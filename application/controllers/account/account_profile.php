...
class Account_profile extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->helper(array('account/ssl', 'url', 'photo'));
	...
	function index($action = NULL)
	{
		...
		$this->form_validation->set_rules(array(array('field' => 'profile_username', 'label' => 'username', 'rules' => 'trim|required|alpha_dash|min_length[2]|max_length[24]')));
		...
		if ($this->form_validation->run())
		{
			...
			{
				$data['profile_username_error'] = 'Username sudah digunakan';
				$error = TRUE;
			}
			else
			{
				...
				default:
					...
					if (isset($_FILES['account_picture_upload']) && $_FILES['account_picture_upload']['error'] != 4)
					{
						...
						else
						{
							...
							$this->image_lib->initialize(array('image_library' => 'gd2', 'source_image' => FCPATH.RES_DIR.'/user/profile/'.$picture['file_name'], 'new_image' => FCPATH.RES_DIR.'/user/profile/pic_'.md5($data['account']->id).$picture['file_ext'], 'maintain_ratio' => FALSE, 'quality' => '100%', 'width' => 200, 'height' => 200));
			...
			if ( ! isset($error)) $data['profile_info'] = 'Profile berhasil di update';
		}
