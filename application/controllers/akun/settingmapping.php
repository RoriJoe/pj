			...
			if($_POST){ 
				...
				foreach($rs->result() as $row){
					...
					<tr>
						...
						<td>$row->tipe $row->nama</td>
						<td style='text-align:center'>
							<a id='$row->id' class='imgupdate btn'>
								<i class='icon-pencil'></i></a><a id='$row->id' class='imgdelete btn'><i class='icon-trash'></i>
							</a>
						</td>
					<tr>";
				}
			...
			if($Attr){
				echo ' Attribute : ';
					foreach($Attr as $HAttr){ $flag=0;
						...
						if($flag==0){
							echo '<input type=checkbox style="margin-left:5px;" class=check value="'.$HAttr->COLUMN_NAME.'" id="" name="Atttabel'.$_POST['idx'].'[]"">'.$HAttr->COLUMN_NAME.'</input>';
						}else{
							echo '<input type=checkbox style="margin-left:5px;" class=check value="'.$HAttr->COLUMN_NAME.'" id="" name="Atttabel'.$_POST['idx'].'[]"" checked>'.$HAttr->COLUMN_NAME.'</input>';
						}
					}
			}
		}
